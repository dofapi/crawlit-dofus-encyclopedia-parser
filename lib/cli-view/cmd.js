...
const firstCategory = [
	...
	},
	{
		...
		choices: ['Equipment', 'Weapon', 'Set', 'Pet', 'Mount', 'Resource', 'Consumable', 'Monster', 'Profession', 'Harness', 'Classe', 'Idols'],
		filter: function (val) {
			return val.toLowerCase();
		}
	}
...
function switchCategory(category) {
	...
	case 'harness':
		...
		case 'classe':
		return categoryClasse();
	case 'idols':
		return categoryIdols();
	default:
...
function categoryIdols() {
	return inquirer.prompt(page).then(answers => {
		answers.language = language;
		answers.game = game;
		answers.category = 'idols';
		return JSON.stringify(answers, null, '  ');
	});
}
