	...
	nightmare
		...
		.evaluate(function () {
			try {
				...
				if (item.url.match(/\barmes\b/gi) || item.url.match(/\bweapons\b/gi)) {
					if($('div.ak-encyclo-detail-right.ak-nocontentpadding').find('div.col-sm-6').eq(0).find('div.ak-list-element').find( "div.ak-title" ).has("span")) {
						item["characteristic"] = [];
					...
					$('div.ak-encyclo-detail-right.ak-nocontentpadding').find('div.col-sm-6').eq(0).find('div.ak-container.ak-panel:not(.no-padding)').find('div.ak-list-element').each(function(i, element){
						var stat = $(this).find( "div.ak-title" ).text().trim();
