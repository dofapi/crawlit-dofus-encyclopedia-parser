...
var getItems = exports.getItems = function(category, links, back) {
	...
	// if(currentPosition == 10) resume.switchErrorHandler('429', gCategory, currentPosition, gLinks, itemsList);
...
function getData(url, back, callback) {
	...
	categoryPromise.then(function(item) {
        ...
        if (err.statusCode == '404') {
        	...
        	resume.switchErrorHandler(err.statusCode, gCategory, currentPosition, gLinks, itemsList);
        }else if(err.code == 'ETIMEDOUT') {
        	resume.switchErrorHandler(err.code, gCategory, currentPosition, gLinks, itemsList);
        }else if(err.message == 'Error: read ECONNRESET') {
        	resume.switchErrorHandler(err.message, gCategory, currentPosition, gLinks, itemsList);
        }else if(err.message == 'Error: unable to verify the first certificate') {
        	resume.switchErrorHandler(err.message, gCategory, currentPosition, gLinks, itemsList);
        }else {
        	console.log(err);
			console.log('\x1b[31m%s\x1b[0m' ,'/!\\Broken promise all');
			resume.switchErrorHandler(err, gCategory, currentPosition, gLinks, itemsList);
        }
      });
}