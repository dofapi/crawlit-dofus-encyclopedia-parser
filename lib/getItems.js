		...
		.evaluate(function () {
			try {
				...
				$('div.ak-encyclo-detail-right.ak-nocontentpadding').find('div.ak-container.ak-panel:not(.no-padding)').find('div.ak-list-element').each(function(i, element){
					...
					var stat = $(this).find( "div.ak-title" ).find("span").before().text().trim();
					console.log('span value : '+$(this).find( "div.ak-title" ).find("span").before().text().trim());
					/*$(this).find( "div.ak-title" ).text().trim();
