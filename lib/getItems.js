...
function getData(url, back, callback) {
	var categoryPromise;
	switch (categoryPromise) {
	  case /\barmes\b/gi.url:
		categoryPromise =  gw.getWeapons(url);
		break;
	  case /\bweapons\b/gi.url:
		categoryPromise = gw.getWeapons(url);
		break;
	  case /\bequipments\b/gi.url:
		categoryPromise = ge.getEquipments(url);
		break;
	  case /\bequipements\b/gi.url:
		categoryPromise = ge.getEquipments(url);
		break;
	  case /\bsets\b/gi.url:
		categoryPromise = gs.getSets(url);
		break;
	  case /\bpanoplies\b/gi.url:
		categoryPromise = gs.getSets(url);
		break;
	  default:
		console.log('Sorry, we are out of ' + categoryPromise + '.');
	}
