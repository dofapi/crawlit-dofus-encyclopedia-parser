...
(async function main() {
	asciiArt();
	arboInit();
	var ifResume = fs.existsSync(process.env.PWD + '/data/links/resume.json') ? true : false;
	try {
...
async function getPageLinks() {
	try {
		...
		fs.writeFileSync(process.env.PWD + '/data/links/' + itemCategory + '_links.json', JSON.stringify(links));
...
async function getItems(category, links, game) {
	try {
		...
		if (fs.existsSync(process.env.PWD + '/data/links/resume.json')) fs.unlinkSync('./data/links/resume.json');
		if (fs.existsSync(process.env.PWD + '/data/links/' + category + '_links.json')) fs.unlinkSync('./data/links/' + category + '_links.json');
		fs.writeFileSync(process.env.PWD + '/data/' + game + '/' + category + '.json', JSON.stringify(items));
...
function getLinksFromFile() {
	var links = JSON.parse(fs.readFileSync(process.env.PWD + '/data/links/' + itemCategory + '_links.json', 'utf8'));
	return links;
}

function resumeLastParse() {
	console.log('\x1b[33m%s\x1b[0m', '/!\\Parsing resumed, let\'s continue the adventure :D');
	var resume = JSON.parse(fs.readFileSync(process.env.PWD + '/data/links/resume.json', 'utf8'));
	getItems(resume.category, resume.links, resume.game);
}

function arboInit() {
	if (!fs.existsSync(process.env.PWD + '/data/dofus-touch') && !fs.existsSync(process.env.PWD + '/data/dofus') && !fs.existsSync(process.env.PWD + '/data/links')) {
		fs.mkdirSync(process.env.PWD + '/data');
		fs.mkdirSync(process.env.PWD + '/data/dofus-touch');
		fs.mkdirSync(process.env.PWD + '/data/dofus');
		fs.mkdirSync(process.env.PWD + '/data/links');
	}
}
