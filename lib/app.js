...
import { parserProcess, parserInit, getPagesLinks } from './modules/global-parser/global-parser';
...
(async function main() {
	...
	try {
		...
		hrstart = process.hrtime(); // to measure execution time
		if (cmdResponse !== 'yes') {
...
function parsingFinished({ numberItemParsed, categoryName }) {
	const hrend = process.hrtime(hrstart); // Result of execution time measure
	...
	console.info('Content parsed in : %ds', hrend[0]);
	process.exit();
}
