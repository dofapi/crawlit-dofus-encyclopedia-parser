...
import { parserProcess, parserInit, getPageLinks } from './modules/global-parser/parser';
import { arboInit, asciiArt } from './services/utils';
...
(async function main() {
	...
	try {
		const cmdResponse = JSON.parse(await getCategory(ifResume));
		if (cmdResponse !== 'yes') {
			await parserInit(cmdResponse);
			await getPageLinks();
