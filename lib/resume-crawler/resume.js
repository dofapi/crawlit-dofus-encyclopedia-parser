...
var switchErrorHandler = exports.switchErrorHandler = function (errorMsg, category, lastIndex, links, items) {
	...
	switch (errorMsg) {
		case '429':
			...
			saveData(category, items, resume);
			wait(4000);
			process.exit();
			break;
		case 'ETIMEDOUT':
...
function saveData(category, items, resume) {
