...
module.exports = {
	...
	recipeParse: (body) => {
		...
		$('div.ak-container.ak-panel.ak-crafts').find('div.ak-panel-content').find('div.ak-container.ak-content-list').find('div.ak-column').each(function (i, element) {
			...
			const setLvl = $(this).find('div.ak-aside').text().replace(/\D/g, '').trim();
	...
	descriptionParse: (body, url) => {
		...
		const $lvlSelector = $('div.ak-encyclo-detail-right').find('div.ak-encyclo-detail-level.col-xs-6.text-right');
		if (typeof $descriptor !== 'undefined') description = $descriptor.text().trim(), description = sanatizer(description);
		if (typeof $lvlSelector !== 'undefined') lvl = $lvlSelector.text().trim().replace(/\D/g, '');
	...
	statsRequest: async (url) => {
		requestPetStatsOpts.url = url;
		const $ = await request(requestPetStatsOpts);
