...
const { effectParse, descriptionParse, statsRequest } = require('../services/parser-helpers');
...
const getPets = exports.getPets = function (url) {
	...
	return request(requestOpts).then(async function ($) {
		...
		if (!url.includes('dofus-touch')) item['statistics'] = effectParse(await statsRequest(url)); // Because Dofus-Touch is using old static pet's stats
