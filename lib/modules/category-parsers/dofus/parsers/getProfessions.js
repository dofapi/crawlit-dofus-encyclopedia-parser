...
import { sanatizer, getId } from '../services/format-helpers';
	...
	return request(requestOpts).then(function ($) {
		...
		if (typeof $akContainer !== 'undefined') {
			$akContainer.find('ul.ak-container.nav.nav-tabs.ak-tabs>li').each(function (i, element) {
				categorySwitch(profession, domain, $(this).eq(0).text().trim().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, ''), $akContainer.find('div.ak-container.ak-tabs-body.tab-content>div').eq(i).html());
			});
		}
