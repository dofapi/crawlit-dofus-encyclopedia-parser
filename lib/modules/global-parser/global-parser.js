...
import { cmdSwitch, cmdSwitchEn } from '../command-line/conf/cmdSwitch';
...
async function parserProcess(category, links, game) {
	try {
		...
		fs.writeFileSync('./data/' + game + '/' + category + '.json', JSON.stringify(items, null, 4));
...
function parserInit(cmdResponse) {
	...
	cmdResponse.language == 'french' ? url = cmdSwitch(itemCategory) : url = cmdSwitchEn(itemCategory);
...
async function getPageLinks() {
	...
	fs.writeFileSync('./data/links/' + itemCategory + '_links.json', JSON.stringify(links));
