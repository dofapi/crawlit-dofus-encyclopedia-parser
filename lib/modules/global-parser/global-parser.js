...
import fs from 'fs';
import { writeFileSync, getQueryString, encodeQueryString } from '../../services/utils';
...
async function getPagesLinks() {
	...
	if (initData.itemCategory == 'classe') {
		...
		// Add links to potential higher levels
		initData.links = initData.links.map(url => {
			const queryString = getQueryString(url);
			const levels = [];

			for (let level = 1 ; level <= 3 ; level++) {
				queryString.level = level;
				levels.push(url.split('?')[0] + encodeQueryString(queryString));
			}
