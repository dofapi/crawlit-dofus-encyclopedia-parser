...
// To choose a conf file corresponding to user's game choice
function gameUrlListSwicher(game) {
	// Game to load depends on cmdResponse from cli choices
	let urlList = undefined;
	try {
		globalConf.games.forEach(gameConf => { if (game === gameConf.key) urlList = gameConf.listUrl; });
		if (!urlList) throw { name: 'gameUrlListSwicher', message: 'No game corresponding in global.conf.js' };
	} catch (error) {
		new errorHandler(error);
	}
	return urlList;
