...
function gameConfParserSwicher(game) {
	...
	try {
		...
		if (!parserList) throw { name: 'gameConfParserSwicher', message: 'No game corresponding to ' + game + ' for ' + game + '.conf.js' };
	} catch (error) {
		new errorHandler(error);
	}
	return parserList;
...
// To choose a conf file corresponding to user's game choice
function gameUrlListSwicher(cmdResponse) {
	// Game to load depends on cmdResponse from cli choices
	let url = undefined;
	try {
		globalConf.games.forEach(game => {
			if (game.key === cmdResponse.game) {
				for (const languageName in game.languages) {
					if (cmdResponse.language === languageName) {
						url = game.languages[languageName](cmdResponse.category);
						break;
					}
				}
			}
		});
		if (!url) throw { name: 'gameUrlListSwicher', message: 'No game corresponding in global.conf.js' };
	} catch (error) {
		new errorHandler(error);
	}
	return url;
...
export { mongoImport, requestOpts, gameConfParserSwicher, gameUrlListSwicher };